-- 1. Tabela de log
CREATE TABLE public.project_sync_log (
...
CREATE INDEX idx_project_sync_log_created_at ON public.project_sync_log(created_at DESC);

ALTER TABLE public.project_sync_log ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated view sync log"
  ON public.project_sync_log FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Managers insert sync log"
  ON public.project_sync_log FOR INSERT
  TO authenticated
  WITH CHECK (has_any_role(auth.uid(), ARRAY['ceo'::app_role, 'executive'::app_role, 'manager'::app_role]));

-- 2. Rescriu sync_project_metrics() ca să logheze
CREATE OR REPLACE FUNCTION public.sync_project_metrics()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF NOT has_any_role(v_user, ARRAY['ceo'::app_role, 'executive'::app_role, 'manager'::app_role]) THEN
  ...
  INSERT INTO public.project_sync_log (source, triggered_by, metrics_synced, success, duration_ms)
  VALUES ('manual', v_user, v_synced, true, EXTRACT(MILLISECONDS FROM (clock_timestamp() - v_started))::int);

  RETURN jsonb_build_object('synced', v_synced, 'at', now());
EXCEPTION WHEN OTHERS THEN
  INSERT INTO public.project_sync_log (source, triggered_by, metrics_synced, success, error_message, duration_ms)
  VALUES ('manual', v_user, v_synced, false, SQLERRM, EXTRACT(MILLISECONDS FROM (clock_timestamp() - v_started))::int);
  RAISE;
END;
$function$;

-- 3. Rescriu sync_project_metrics_internal() (cron) ca să logheze
...
BEGIN
  ...
  INSERT INTO public.project_sync_log (source, triggered_by, metrics_synced, success, duration_ms)
  VALUES ('cron', NULL, v_synced, true, EXTRACT(MILLISECONDS FROM (clock_timestamp() - v_started))::int);

  RETURN jsonb_build_object('synced', v_synced, 'at', now());
EXCEPTION WHEN OTHERS THEN
  INSERT INTO public.project_sync_log (source, triggered_by, metrics_synced, success, error_message, duration_ms)
  VALUES ('cron', NULL, v_synced, false, SQLERRM, EXTRACT(MILLISECONDS FROM (clock_timestamp() - v_started))::int);
  RAISE;
END;
$function$;